    ...
    <div class="flex flex-col w-full max-w-[776px] gap-6">
      ...
      <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div class="flex items-center gap-4">
          ...
          <h1 class="text-2xl font-semibold tracking-tight text-foreground font-geist text-[#27272B]">
            Personal Details
          </h1>
        </div>
      ...
      <Card class="rounded-xl border p-4 sm:p-6 bg-white">
        <div class="flex flex-col gap-6">
          <h2 class="text-lg font-semibold text-foreground font-geist">Personal Details</h2>
          ...
          <!-- New Email Field -->
          <div class="flex flex-col gap-2">
            <Label for="newEmail" class="text-sm font-medium text-foreground font-inter">
              New Email
            </Label>
            <Input
              id="newEmail"
            ...
            />
            <p v-if="errors.newEmail" class="text-sm text-red-600">{{ errors.newEmail }}</p>
          </div>

          <!-- Password Field -->
          <div class="flex flex-col gap-2">
            <Label for="password" class="text-sm font-medium text-foreground font-inter">
              Current Password
            </Label>
            <div class="relative">
              <Input
            ...
            <p class="text-xs text-gray-500">Enter your current password to confirm the email change</p>
          </div>

          <!-- Language Field -->
          <div class="flex flex-col gap-2">
            <Label for="language" class="text-sm font-medium text-foreground font-inter">
              Language
            </Label>
...
const isFormValid = computed(() => {
  const hasEmailChange = newEmail.value.trim() !== '' && 
...
const validateForm = () => {
  ...
  if (!newEmail.value.trim()) {
    ...
    isValid = false
  } else if (newEmail.value.trim().toLowerCase() === currentEmail.value.toLowerCase()) {
...
const handleSaveChanges = async () => {
  ...
  if (!currentUser.value || !currentUser.value.id) {
    ...
    // Final check
    if (!currentUser.value || !currentUser.value.id) {
      errorMessage.value = 'You must be logged in to change your email. Please refresh the page and try again.'
      return
    }
  }

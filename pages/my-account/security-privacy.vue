    ...
    <div class="flex flex-col w-full max-w-[776px] gap-6">
      ...
      <Card class="rounded-xl border p-4 sm:p-6 bg-white">
        <div class="flex flex-col gap-6">
          <h2 class="text-lg font-semibold text-foreground font-geist">Change Password</h2>

          <!-- Current Password Field -->
          <div class="flex flex-col gap-2">
            <Label for="currentPassword" class="text-sm font-medium text-foreground font-inter">
              Current Password
            </Label>
            <div class="relative">
              <Input
          ...
          <!-- New Password Field -->
          <div class="flex flex-col gap-2">
            <Label for="newPassword" class="text-sm font-medium text-foreground font-inter">
              New Password
            </Label>
            <div class="relative">
              <Input
          ...
          <div class="flex flex-col gap-2">
            <Label for="repeatPassword" class="text-sm font-medium text-foreground font-inter">
              Repeat New Password
            </Label>
            <div class="relative">
              <Input
            ...
            <p v-if="errors.repeatPassword" class="text-sm text-red-600">{{ errors.repeatPassword }}</p>
          </div>
        </div>
      </Card>
    </div>
...
const handleSaveChanges = async () => {
  ...
  if (!currentUser.value || !currentUser.value.id) {
    ...
    // Final check
    if (!currentUser.value || !currentUser.value.id) {
      errorMessage.value = 'You must be logged in to change your password. Please refresh the page and try again.'
      return
    }
  }

  console.log('🔵 Attempting to change password for user:', currentUser.value.id)
  loading.value = true

 ...
 <div class="min-h-screen py-10 pb-32 px-52">
    <div class="max-w-4xl mx-auto space-y-6">
      ...
      <div class="flex items-center justify-between">
       <h1 class="text-2xl font-semibold leading-8" style="font-family: Geist; color: #020617; letter-spacing: -0.006em;">
      ...
      <!-- Empty State -->
      <div v-else-if="!fields || fields.length === 0" class="bg-white rounded-xl shadow-sm p-12 text-center border">
        <div class="max-w-md mx-auto">
          ...
          <h3 class="text-xl font-semibold text-gray-900 mb-2">No Additional Information Required</h3>
          <p class="text-gray-600 mb-6">There are no additional fields to fill for this application.</p>
        </div>
        </div>
      ...
      <div v-else class="space-y-6">
        <!-- Application Info -->
        <div v-if="application" class="bg-white rounded-xl border p-4" style="border-color: #E4E4E8;">
          <h2 class="text-base font-semibold leading-4" style="font-family: Geist; color: #27272B;">
            {{ application.destinationCountry || 'Visa Application' }}
          </h2>
          <p class="text-sm text-gray-600 mt-1">Application #{{ application.applicationNumber }}</p>
        </div>
...
>
  <div class="flex items-start gap-3">
    ...
    <div>
      <p class="font-medium" style="font-family: Geist;">Additional information requested</p>
      <p class="text-sm mt-1" style="font-family: Inter;">{{ adminNoteToShow }}</p>
    </div>
  </div>
</div>
        ...
        <div class="bg-white rounded-xl border overflow-hidden" style="border-color: #E4E4E8;">
          ...
          <div class="border-b flex overflow-x-auto" style="border-color: #E4E4E8;">
            <button
              ...
              :class="[
                selectedTravelerId === (traveler.id || null) && selectedTravelerIndex === index
          ...
          <div class="p-6">
            <form @submit.prevent="handleSubmit" class="space-y-6">
              ...
              <div class="pb-4 border-b" style="border-color: #E4E4E8;">
                <h3 class="text-lg font-semibold" style="font-family: Geist; color: #020617;">
                  {{ getTravelerName(currentTraveler, selectedTravelerIndex) }}
                </h3>
                <p class="text-sm text-gray-600 mt-1">
                  {{ selectedTravelerId ? 'Traveler Information' : 'Application Information' }}
                </p>
              </div>
              ...
              <div v-for="field in sortedFields" :key="field.id" class="space-y-2">
                ...
                >
                  ...
                  <span v-if="shouldHighlightField(field)" class="ml-2 text-xs font-medium text-red-600">Requested</span>
                </label>
               ...
               <Input
                  ...
                  :class="shouldHighlightField(field) ? 'border-red-500 ring-1 ring-red-200' : ''"
            ...
            />
              ...
              />
                ...
                <!-- Date Input -->
                <div v-else-if="field.fieldType === 'date'" class="grid grid-cols-3 gap-4">
                  <Select v-model="formResponses[field.id].date.day" :disabled="isFieldDisabled(field)" :class="shouldHighlightField(field) ? 'border-red-500 ring-1 ring-red-200 rounded-md' : ''">
                  ...
                  <Select v-model="formResponses[field.id].date.month" :disabled="isFieldDisabled(field)" :class="shouldHighlightField(field) ? 'border-red-500 ring-1 ring-red-200 rounded-md' : ''">
                    <SelectTrigger variant="form">
                  ...
                  <Select v-model="formResponses[field.id].date.year" :disabled="isFieldDisabled(field)" :class="shouldHighlightField(field) ? 'border-red-500 ring-1 ring-red-200 rounded-md' : ''">
                    <SelectTrigger variant="form">
              ...
              :required="field.isRequired"
              :disabled="isFieldDisabled(field)"
              :class="shouldHighlightField(field) ? 'border-red-500 ring-1 ring-red-200 rounded-md' : ''"
            >
              <SelectTrigger variant="form">
...
<div v-else-if="field.fieldType === 'upload'" class="relative">
  <input
    ...
    :ref="(el) => { if (el) setFileInputRef(field.id, el) }"
    @change="(e) => handleFileUpload(e, field.id)"
    class="hidden"
    :accept="field.allowedFileTypes?.join(',')"
    :required="field.isRequired && !formResponses[field.id]?.filePath"
    :disabled="isFieldDisabled(field)"
  />
  <button
    type="button"
    @click="triggerFileUpload(field.id)"
    :disabled="formResponses[field.id]?.isUploading || isFieldDisabled(field)"
    ...
    :class="[
      formResponses[field.id]?.isUploaded 
        ...
        : (isFieldDisabled(field) ? 'text-gray-400 bg-gray-50 border-gray-300 cursor-not-allowed' : 'text-gray-500 hover:bg-gray-50 cursor-pointer border-[#D4D4DA]'),
      shouldHighlightField(field) ? 'border-red-500 ring-1 ring-red-200' : ''
    ]"
  >
    <div class="flex items-center gap-2">
      <!-- Loading Spinner -->
      ...
      <span>
        <span v-if="formResponses[field.id]?.isUploading">Uploading...</span>
        <span v-else-if="formResponses[field.id]?.isUploaded && formResponses[field.id]?.fileName">
  ...
  <!-- Success Message -->
  <p v-if="formResponses[field.id]?.isUploaded && !formResponses[field.id]?.uploadError" class="text-green-600 text-xs mt-1 flex items-center gap-1">
    <svg class="h-3 w-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path>
    </svg>
    File uploaded successfully
  </p>
  <p v-if="formResponses[field.id]?.uploadError" class="text-red-500 text-xs mt-1">
    {{ formResponses[field.id].uploadError }}
  </p>
  <p v-if="field.allowedFileTypes && field.allowedFileTypes.length > 0" class="text-gray-500 text-xs mt-1">
  ...
  <p v-if="field.maxFileSize" class="text-gray-500 text-xs mt-1">
    Max size: {{ formatFileSize(field.maxFileSize) }}
              ...
              <!-- Submit Button -->
              <div class="flex justify-end pt-4 border-t" style="border-color: #E4E4E8;">
                <Button
                  @click="handleSubmit"
                  class="h-9 px-4 rounded-md text-white font-medium text-sm"
                  style="background-color: #1ECE84; font-family: Geist; min-width: 100px;"
...
import { useVisaProductFieldsApi, type VisaProductFieldWithResponse } from '@/composables/useVisaProductFields'
import { useVisaApplications, type Traveler } from '@/composables/useVisaApplications'
import { useApi } from '@/composables/useApi'

const route = useRoute()
const router = useRouter()
const { getFieldsByApplication, submitFieldResponses, checkAllAdditionalInfoSubmitted } = useVisaProductFieldsApi()
...
const formResponses = reactive<Record<number, {
  value: string | null
...
const submissionStatus = reactive<Record<string, 'submitted' | 'pending' | 'not_started'>>({})
...
const allTravelers = computed(() => {
  const result: Array<{ id?: number; firstName?: string; lastName?: string; isApplication?: boolean }> = [
...
const currentTraveler = computed(() => {
  ...
  return travelers.value.find(t => t.id === selectedTravelerId.value) || { isApplication: true }
})
  ...
  if (app.resubmissionRequests && Array.isArray(app.resubmissionRequests)) {
    const activeRequests = app.resubmissionRequests.filter((req: any) => {
      ...
      if (!selectedTravelerId.value) {
        ...
        return req.target === 'traveler' && req.travelerId === selectedTravelerId.value
      }
    })
...
const getTravelerName = (traveler: any, index: number): string => {
  ...
  if (traveler.firstName || traveler.lastName) {
    return `${traveler.firstName || ''} ${traveler.lastName || ''}`.trim() || `Traveler ${index}`
  }
  return `Traveler ${index}`
}

const getTravelerStatus = (travelerId: number | null, index: number): string => {
...
const getTravelerStatusClass = (travelerId: number | null, index: number): string => {
  const key = travelerId ? `traveler-${travelerId}` : 'application'
  const status = submissionStatus[key] || 'not_started'
...
const checkSubmissionStatus = async (travelerId: number | null) => {
  ...
  try {
    const response = await getFieldsByApplication(applicationId.value, travelerId || undefined)
        ...
        if (!formResponses[field.id]) {
          ...
          if (field.response) {
            if (field.fieldType === 'date' && field.response.value) {
              ...
              formResponses[field.id].filePath = field.response.filePath || null
              formResponses[field.id].fileName = field.response.fileName || null
              formResponses[field.id].fileSize = field.response.fileSize || null
...
const triggerFileUpload = (fieldId: number) => {
  ...
  if (fileInput) {
    ...
    const fallbackInput = document.getElementById(`file-input-${fieldId}`) as HTMLInputElement
    if (fallbackInput) {
      fallbackInput.click()
    } else {
...
const handleFileUpload = async (event: Event, fieldId: number) => {
  ...
  if (field.allowedFileTypes && field.allowedFileTypes.length > 0) {
    ...
    const allowedExtensions = field.allowedFileTypes.map(type => type.toLowerCase())
    if (!allowedExtensions.some(ext => fileExtension === ext.toLowerCase() || file.type === ext)) {
      formResponses[fieldId].uploadError = `File type not allowed. Allowed types: ${field.allowedFileTypes.join(', ')}`
      return
    }
  }
  ...
  if (field.maxFileSize && file.size > field.maxFileSize) {
    formResponses[fieldId].uploadError = `File size exceeds maximum allowed size of ${formatFileSize(field.maxFileSize)}`
    return
  }
  ...
  try {
    ...
    formResponses[fieldId].uploadError = err.message || 'File upload failed. Please check if the upload endpoint is configured correctly.'
    formResponses[fieldId].isUploading = false
    formResponses[fieldId].isUploaded = false
  }
}
  ...
  fields.value.forEach(field => {
    ...
    if (field.isRequired) {
      if (field.fieldType === 'upload') {
        if (!formResponses[field.id]?.filePath && !formResponses[field.id]?.file) {
        ...
        if (!formResponses[field.id]?.value || formResponses[field.id].value.trim() === '') {
          fieldErrors[field.id] = 'This field is required'
          isValid = false
        }
      }
    ...
    if (field.fieldType === 'dropdown' && field.options && formResponses[field.id]?.value) {
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (submitResponse.success) {
      ...
      const key = selectedTravelerId.value ? `traveler-${selectedTravelerId.value}` : 'application'
      submissionStatus[key] = 'submitted'
      ...
      if (allSubmitted) {
        alert('All additional information submitted successfully! Your application status has been updated.')
        // Redirect after user clicks OK on alert
        router.push('/my-account/my-orders')
      } else {
        alert('Additional information submitted successfully! Please complete the remaining forms.')
      }
    } else {
...
const initializeData = async () => {
  if (!applicationId.value) {
    error.value = 'Application ID is required. Please provide it in the URL query parameter.'
    return
  }
  ...
  try {
    ...
    if (app) {
      ...
      if (!hasActiveResubmission && !hasLegacyResubmission) {
        ...
        console.log('⚠️ No active resubmission requests, redirecting to my-orders')
    ...
    if (app && app.resubmissionRequests && Array.isArray(app.resubmissionRequests)) {
      const activeRequests = app.resubmissionRequests.filter((req: any) => !req.fulfilledAt)
      ...
      if (activeRequests.length > 0) {
        ...
        if (firstRequest.target === 'application') {
          ...
          const idx = travelers.value.findIndex(t => t.id === firstRequest.travelerId)
          if (idx !== -1) {
...
watch(isResubmissionActive, (isActive) => {
  if (!isActive && application.value) {
    ...
    if (!hasLegacyResubmission) {
      console.log('⚠️ Resubmission no longer active, redirecting to my-orders')
      navigateTo(`/my-account/my-orders/${applicationId.value}`)
    }
  }
}, { immediate: false })
...
watch(() => route.query.applicationId, async (newApplicationId) => {
  if (newApplicationId && String(newApplicationId) === String(applicationId.value)) {

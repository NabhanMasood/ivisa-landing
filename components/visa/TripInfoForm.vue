  ...
  <div class="border rounded-xl p-6" style="border-color: #E5E7EB;">
    ...
    <div class="space-y-6">
      ...
      <div>
        ...
        <!-- Loading State -->
        <div v-if="isLoadingCountries" class="mt-2 flex items-center gap-2 text-sm text-gray-600">
      ...
      <div>
        ...
        <!-- Loading State -->
        <div v-if="isLoadingProducts" class="mt-2 flex items-center gap-2 text-sm text-gray-600">
          <div class="w-4 h-4 border-2 border-gray-300 border-t-black rounded-full animate-spin"></div>
        ...
        <Select v-else v-model="formData.visaType">
          ...
          <SelectContent>
            <SelectItem 
                ...
                >
                {{ product.productName }} - {{ formatPrice(Number(product.totalAmount)) }} ({{ product.entryType }}, {{ product.duration }} days, Valid {{ product.validity }} days)
            </SelectItem>
          </SelectContent>
        </Select>
        
        <!-- No products message -->
        <p v-if="!isLoadingProducts && availableProducts.length === 0 && !productError" class="mt-2 text-sm text-gray-500">
      ...
      <Button 
        @click="handleNext"
        :disabled="!formData.visaType || isLoadingProducts || availableProducts.length === 0"
        class="w-[143px] h-9"
...
const { formatPrice, initializeRates, selectedCurrency, getCurrentRate } = useCurrency()
...
const selectedCountry = computed(() => {
  return nationalityOptions.value.find(c => c.countryName === formData.value.nationality)
})
...
const fetchNationalityOptions = async () => {
  ...
  try {
    ...
    if (response.success && response.data) {
      ...
      console.log('✅ Loaded database countries:', nationalityOptions.value.length)
    }
  } catch (error) {
  ...
  try {
    ...
    if (productId) {
      ...
      if (response.success && response.data && response.data.processingFees) {
        console.log(`✅ Found ${response.data.processingFees.length} processing fees for product ${productId}`)
...
const fetchVisaProducts = async (nationality: string, destination: string) => {
  ...
  try {
    ...
    const response = await getNationalityDestinationProducts(nationality, destination)
    ...
    if (response.success && response.data) {
      ...
      const uniqueProducts = response.data.reduce((acc, product) => {
        ...
        if (!existing) {
          ...
          console.log('⚠️ Duplicate found, skipping:', product.productName, product.entryType)
        }
        return acc
      ...
      const enrichedProducts = await Promise.all(
        uniqueProducts.map(product => enrichProductWithProcessingFees(product))
      )
      
      availableProducts.value = enrichedProducts
      
      console.log('✅ Products loaded with processing fees:', availableProducts.value.length)
      console.log('📊 Product details:', enrichedProducts.map(p => ({
        name: p.productName,
      ...
      if (previousVisaType && enrichedProducts.some(p => `${p.productName}|${p.entryType}` === previousVisaType)) {
      ...
      else if (props.initialData?.visaType && enrichedProducts.some(p => `${p.productName}|${p.entryType}` === props.initialData.visaType)) {
        formData.value.visaType = props.initialData.visaType
        console.log('✅ Restored from initialData:', props.initialData.visaType)
      }
      // ✅ PRIORITY 3: Auto-select first product only if no previous selection
        ...
        productError.value = 'No visa products available for this nationality-destination combination'
      }
    } else {
...
const selectedProduct = computed(() => {
  ...
  return availableProducts.value.find(p => 
...
onMounted(async () => {
  ...
  if (props.initialData) {
    ...
    const actualTravelersCount = props.initialData.travelers?.length || props.initialData.applicants || 1

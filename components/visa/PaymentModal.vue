    ...
    <Dialog v-if="!showSuccessModal" :open="isOpen && !showSuccessModal" @update:open="handleOpenChange">
      <DialogContent class="w-[440px]">
        <DialogHeader>
          ...
          <DialogDescription>
            <span v-if="isAuthenticated && savedCards.length > 0 && paymentMethod === 'saved' && !showOtherCards">
              Your default card is selected
            </span>
            <span v-else-if="isAuthenticated && savedCards.length > 0 && paymentMethod === 'saved' && showOtherCards">
              Select a saved card
            </span>
            <span v-else>
        ...
        <div v-if="isAuthenticated && !loadingCards && savedCards.length > 0 && paymentMethod === 'saved' && !showOtherCards" class="space-y-4">
          <!-- Default/Selected Card Display -->
          <div class="p-4 border-2 rounded-lg" :style="{ borderColor: '#1ECE84' }">
            <div class="flex items-center justify-between mb-2">
              <div class="flex items-center gap-2">
                <span class="text-sm font-semibold text-gray-700">
                  {{ getSelectedCard()?.cardBrand.toUpperCase() }} •••• {{ getSelectedCard()?.cardLast4 }}
                </span>
                <span v-if="getSelectedCard()?.isDefault" class="px-2 py-0.5 text-xs font-medium rounded" style="background-color: #1ECE84; color: white;">
                  Default
                </span>
              </div>
            </div>
            <p class="text-xs text-gray-500">
              Expires {{ formatCardExpiryDisplay(getSelectedCard()?.expiryMonth || '', getSelectedCard()?.expiryYear || '') }}
            </p>
          ...
          <div class="flex flex-col gap-2">
            ...
            <Button
              type="button"
              variant="outline"
              @click="paymentMethod = 'new'; showOtherCards = false"
              class="w-full"
              style="border-color: #1ECE84; color: #1ECE84;"
            >
              Use a new card
        ...
        <div v-if="isAuthenticated && !loadingCards && savedCards.length > 0 && paymentMethod === 'saved' && showOtherCards" class="space-y-4">
          ...
          <div class="space-y-2 max-h-60 overflow-y-auto">
            <div
              ...
              :key="card.id"
              @click="selectedCardId = card.id; showOtherCards = false"
              class="p-3 border-2 rounded-lg cursor-pointer transition-colors"
              :class="selectedCardId === card.id ? 'bg-green-50' : 'hover:bg-gray-50'"
              :style="{ borderColor: selectedCardId === card.id ? '#1ECE84' : '#e5e7eb' }"
            >
              <div class="flex items-center justify-between">
                <div class="flex items-center gap-2">
                  <span class="text-sm font-medium" :class="selectedCardId === card.id ? 'text-gray-900' : 'text-gray-700'">
                    {{ card.cardBrand.toUpperCase() }} •••• {{ card.cardLast4 }}
                  </span>
                  <span v-if="card.isDefault" class="px-2 py-0.5 text-xs font-medium rounded" style="background-color: #1ECE84; color: white;">
                    Default
                  </span>
                  <span v-if="selectedCardId === card.id" class="px-2 py-0.5 text-xs font-medium rounded" style="background-color: #1ECE84; color: white;">
                    Selected
                  </span>
                </div>
                <input
              ...
              <p class="text-xs text-gray-500 mt-1">
                Expires {{ formatCardExpiryDisplay(card.expiryMonth, card.expiryYear) }} • {{ card.cardholderName }}
              </p>
            </div>
          </div>
        ...
        <div v-if="!loadingCards && (paymentMethod === 'new' || (isAuthenticated && savedCards.length === 0) || !isAuthenticated || (paymentMethod === 'saved' && savedCards.length > 0))" class="space-y-4">
          <!-- Note for saved cards -->
          <div v-if="paymentMethod === 'saved' && savedCards.length > 0" class="p-3 bg-blue-50 border border-blue-200 rounded-md">
            <p class="text-sm text-blue-800">
              <strong>Note:</strong> Please enter your card details below to complete the payment with Stripe.
            </p>
          </div>
          ...
          <div class="relative">
            <Input
              ...
              :required="paymentMethod === 'new' || savedCards.length === 0 || paymentMethod === 'saved'"
              class="pr-20 h-12"
          ...
          <!-- Stripe Card Element -->
          <div id="card-element" class="h-12 px-3 py-2 border rounded-md" style="border: 1px solid #1ECE84;"></div>
          <div v-if="cardErrors" id="card-errors" role="alert" class="text-sm text-red-600 mt-1">{{ cardErrors }}</div>
        </div>
...
const loadSavedCards = async () => {
  ...
  try {
    ...
    if (response.success && response.data?.data) {
      ...
      const cards: CardInfo[] = Array.isArray(response.data.data) ? response.data.data : []
      ...
      console.warn('⚠️ Response details:', { success: response.success, hasData: !!response.data, hasNestedData: !!response.data?.data })
      savedCards.value = []
    }
  } catch (error) {
  ...
  if (isAuthenticated.value) {
    ...
    if (savedCards.value.length > 0) {
      ...
      if (defaultCard) {
        ...
        console.log('✅ No default card, using first card:', savedCards.value[0].id)
        paymentMethod.value = 'saved'
...
watch(() => paymentMethod.value, async (newMethod) => {
  if (props.isOpen) {
    ...
    setTimeout(() => {
      ...
      if (newMethod === 'new' || (newMethod === 'saved' && savedCards.value.length > 0)) {
        initializeStripeElements()
      } else {
        cleanupStripeElements()
      }
...
const transformApplicationData = (applicationData: any) => {
  ...
  console.log('📞 Travelers data with phone:', travelersWithPhone.map((t: any) => ({
  ...
  if (isAuthenticated.value && currentUser.value?.id) {
    ...
    console.log('✅ Including customerId from logged-in user:', payload.customerId)
  } else {
    console.log('⚠️ User not logged in, backend will create/find customer based on email')
  }

  console.log('✅ Final payload:', payload)
  console.log('📞 Phone numbers in payload travelers:', payload.travelers.map((t: any) => ({
  ...
  if (payload.travelers.length > 0 && payload.travelers[0]) {
    ...
    if (!firstTraveler.phone || firstTraveler.phone.trim() === '') {
      ...
      console.error('First traveler data:', JSON.stringify(firstTraveler, null, 2))
    } else {
      console.log('✅ Phone number validated for first traveler:', firstTraveler.phone)
      console.log('✅ Full first traveler object:', JSON.stringify(firstTraveler, null, 2))
    }
  }
...
const handlePayment = async () => {
  ...
  try {
    ...
    console.log('💳 Creating payment intent for amount:', amountInCents, 'cents (', transformedData.totalAmount, 'dollars)')
    ...
    if (paymentMethod.value === 'saved' && selectedCardId.value && selectedCardId.value !== '') {
      ...
      if (!cardElement.value) {
        throw new Error('Card element not initialized. Please enter card details.')
      }
    ...
    if (transformedData.travelers && transformedData.travelers.length > 0) {
      transformedData.travelers.forEach((t: any, index: number) => {
        ...
        if (index === 0 && (!t.phone || t.phone.trim() === '')) {
          console.error('❌ CRITICAL: First traveler phone is missing or empty!')
        }
      })
    }
    ...
    if (process.client) {
      try {
        ...
        const destinationCountry = props.applicationData.destinationCountry || 'Morocco'
    ...
    if (error.message && error.message.includes('duplicate key')) {
      alert('This payment has already been processed. Please check your orders or try again with a new application.')
    } else {
